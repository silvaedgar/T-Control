...
class Client extends Model
{
    ...
    public function scopeGetClients($query, $filter = [])
    {
        return $query->where($filter)->orderBy('names');
    }
    ...
    //     }
    //     if ($filter == '') {
    //         return $query->where('activo', 1)->orderBy('names');
    //     }
    //     return $query->where('activo', $filter)->orderBy('names');
    // }
    ...
    public function scopeGetDataSales($query,$id,$calc_coin_id,$base_coin_id)
    {
        ...
        return  Client::select(
            'sale_date as date', 'sales.id', 'sales.created_at as create', 'mount',
            ...
            ->selectRaw("'sales.show' as route")
            ->selectRaw($column_balance_sales)
    ...
    public function scopeGetDataPayments($query,$id,$calc_coin_id,$base_coin_id)
    {
        ...
        return  Client::select(
            'payment_date as date', 'payment_clients.id', 'payment_clients.created_at as create',
            'mount', 'rate_exchange', 'balance', 'clients.id as client_id', 'clients.count_in_bs', 'clients.names',
            'coins.symbol', 'coins.id as coin_id'
        )
            ->selectRaw("'Pagos' as type")
            ->selectRaw("'paymentclients.show' as route")
