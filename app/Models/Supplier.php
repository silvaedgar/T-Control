...
class Supplier extends Model
{
    ...
    protected $fillable = ['user_id', 'document_type', 'document', 'name', 'contact', 'address', 'phone', 'balance', 'status'];
    ...
    {
        return $query->where($filter)->orderBy('name');
    }
    ...
    public function scopeGetDataPurchases($query, $id, $calc_coin_id, $base_coin_id, $filter = '')
    {
        ...
        return Supplier::select('purchase_date as date', 'mount', 'balance', 'rate_exchange', 'purchases.id', 'purchases.created_at as create', 'suppliers.name', 'suppliers.id as supplier_id', 'coins.symbol', 'coins.id as coin_id')
            ->selectRaw(" 'N' as count_in_bs")
            ->selectRaw("'Compras' as type")
            ->selectRaw("'purchases.show' as route")
            ->selectRaw($column_balance_purchases)
    ...
    public function scopeGetDataPayments($query, $id, $calc_coin_id, $base_coin_id)
    {
        ...
        return Supplier::select('payment_date as date', 'mount', 'balance', 'rate_exchange', 'payment_suppliers.id', 'payment_suppliers.created_at as create', 'suppliers.name', 'suppliers.id as supplier_id', 'coins.symbol', 'coins.id as coin_id')
            ->selectRaw("'N' as count_in_bs")
            ->selectRaw("'Pagos' as type")
            ->selectRaw("'paymentsuppliers.show' as route")
            ->selectRaw($column_balance_payments)
