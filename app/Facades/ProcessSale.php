...
class ProcessSale {
    ...
    public function getData($id=0) {
        ...
        if ($id > 0) {
            $data['invoice'] = Sale::with('Client','Coin','SaleDetails','SaleDetails.Product')->where('id',$id)->first();
    ...
    public function storeSale(StoreSaleRequest $request) {
        DB::beginTransaction();
            try {
                $data_coin = $this->generate_data_coin('calc_currency_sale');
                ...
                if($request->conditions == "Credito") {
                    ...
                    if ($client->count_in_bs == 'S')
                        $balance_client += ($request->coin_id == $data_coin['base_coin']->id
                            ? $mount : $mount * $rate_exchange);
                    else
                        $balance_client += ($request->coin_id == $data_coin['calc_coin']->id
                            ? $mount : $mount / $rate_exchange);
