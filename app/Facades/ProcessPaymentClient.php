...
class ProcessPaymentClient {
    ...
    private function calcBalanceClient(StorePaymentClientRequest $request,Client $client,$rate_exchange)
    {
        $paymentcurrency = $request->coin_id;
        if ($client->count_in_bs == 'S') {
            if ($request->coin_id != 1 )  /// ojo tiene que ser calc_coin_id
    ...
    private function calc_balance_sale(PaymentClient $request,Sale $sale,$mount_pending, $convert_mount_payment) {
        ...
        if ($mount_pending >= $balance_sale) {
            return  ['paid_mount' => $balance_sale,'status' => 'Cancelada', 'opcion' => $convert_mount_payment,
                        'mount_pending' => $this->calc_mount_pending($request,$mount_pending - $balance_sale, $convert_mount_payment)];
        }
    ...
    private function verify_data_sale(PaymentClient $payment_client,Sale $sale,$calc_coin_current_id, $mount_pending)
    {
        ...
        $balance_sale = $sale->mount - $sale->paid_mount;
        if ($payment_currency == $sale->coin_id){ //Moneda de Pago es igual a la FC
    ...
    public function getDataPayment($id=0) {
        ...
        if ($id > 0) {
            $data['invoice'] = PaymentClient::with('Client','Coin','PaymentForm')->where('id',$id)->first();
            $data['mount_other'] = ($data['calc_coin']->id != $data['base_coin']->coin_id ? $this->mount_other($data['invoice'],$data['calc_coin']) : 0);
        }
        $data['clients'] = $id > 0 ? array($data['invoice']->client) : Client::GetClients()->get();
        $data['payment_forms'] = $id > 0 ? array($data['invoice']->PaymentForm) : PaymentForm::where('status','Activo')->orderBy('payment_form')->get();
        // $rate = $this->get_base_coin_rate($data['calc_coin']->id);
        $data['coins'] = $id > 0 ? array($data['invoice']->coin) : $this->get_coins_invoice_payment($this->get_base_coin_rate($data['calc_coin']->id),'calc_currency_sale')->get();
        // $data['coins'] = $id > 0 ? array($data['invoice']->coin) : $this->get_coins_invoice_payment($rate,'calc_currency_sale')->get();
        // $data['rate'] = $rate->first();

        $data['header'] = $id > 0 ? "Pago de Cliente" : "Crear Pago de Clientes";
        return $data;
    }

    public function storePayment(StorePaymentClientRequest $request) {
        ...
        try {
            ...
            foreach ($sale_pendings as $key => $value) {
                if ($monto > 0) {
                    array_push($sale_update,$this->verify_data_sale($payment_client,
                             $sale_pendings[$key],$request->calc_currency_id,$monto));
            ...
            DB::rollback();
            // echo $th;
            $message =  "No se proceso Pago del Cliente: $client->names por un Monto de: ".number_format($request->mount,2)." "
