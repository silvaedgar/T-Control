...
class PaymentClientController extends Controller
{
    ...
    public function createfilter($data) {
        ...
        if (!empty($data->startdate)) {
            $filtro = ['payment_date','>=',$data->startdate];
            array_push($filter,$filtro);
        }
        if (!empty($data->enddate)) {
            $filtro = ['payment_date','<=',$data->enddate];
            array_push($filter,$filtro);
        }
        return $filter;
    ...
    public function filterpayment(Request $request) {
        ...
        $paymentclients = $this->index($filter);
        return view('payment-clients.index',compact('paymentclients','filter'));
    }

    public function index($filter = [])
    {
        if (count($filter) == 0)
            $paymentclients = PaymentClient::orderBy('payment_date','desc')->orderBy('created_at','desc')->where('status','<>','Historico')->get();
        else
            return PaymentClient::where($filter)->where('status','<>','Historico')->orderBy('payment_date','desc')->orderBy('created_at','desc')->get();
    ...
    public function store(StorePaymentClientRequest $request)
    {
        ...
        try {
            ...
            foreach ($sale_pendings as $key => $value) {
                if ($monto > 0) {
// ------------------------------ Todas estas lineas no estan en supplier -----------------
                    ...
                    else {  // OJO HAY QUE REVISAR LO DE MULTIPLICAR O DIVIDIR
...
// ------------------------------ hasta aqui ---------------------------------------------
    ...
    public function report(Request $request) {
        ...
        $filter = $this->createfilter($request);
        $paymentclients = $this->index($filter);
        $pdf = PDF::loadView('payment-clients.report',['paymentclients' => $paymentclients,'filter'=> $filter]);
        return $pdf->stream();
    }
