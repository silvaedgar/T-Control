...
class CoinBaseController extends Controller
{
    ...
    private function FindRateExchange($currency_calc_old, $currency_calc_new, $price)
    {
        ...
        $coin_values = CurrencyValue::select($columna)
            ->where('base_currency_id', $currency_calc_old)
            ->where('coin_id', $currency_calc_new)
            ->where('status', 'Activo')
            ->first();
        ...
        try {
            ...
            if ($calccoinpurchase != '') {
                if ($calccoinpurchase->id != $request->calc_currency_purchase) {
                    $rate_exchange = $this->FindRateExchange($calccoinpurchase->id, $request->calc_currency_purchase, 'purchase_price');
                    if ($rate_exchange < 0) {
                        DB::rollback();
                        return redirect()
                            ->route('coinbase.index')
                            ->with('status', 'Error_No existe valor de divisa relacionado entre la anterior y la nueva. Verifique');
                    }
                    Supplier::update([
