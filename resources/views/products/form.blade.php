<input type="hidden" value="{{ isset($data['product']) ? old('id', $data['product']->id) : 0 }}" name="id">
<input type="hidden"
    value="{{ isset($data['product']) ? old('user_id', $data['product']->user_id) : auth()->user()->id }}" name="user_id">
{{-- <input type="hidden" value="{{ isset($data['product']) ? old('category_id', $data['product']->category_id) : 0 }}"
    id="category_id"> --}}
...
<div class="row mt-3">
    <div class="col-sm-10">
        <div class="row">
            ...
            <div class="col-sm-9 col-md-3 ml-sm-0 " style="margin-top:-5px">
                <input class="form-control " name="code" id="input-code" type="text"
                    ...
                    value="{{ old('code', isset($data['product']) ? $data['product']->code : '') }}" required
                    aria-required="false">
                @error('code')
            ...
            <div class="col-sm-8 col-md-6" style="margin-top:-5px">
                <input class="form-control" name="name" id="input-name" type="text"
                    ...
                    value="{{ old('name', isset($data['product']) ? $data['product']->name : '') }}" required
                    aria-required="false">
                @error('name')
                    <span class="text-danger"> {{ $message }} </span> <br />
                @enderror
        ...
        <div class="row mt-2">
            ...
            <div class="col-sm-7 col-md-5 col-xl-4">
                <select name="category_id" id="category_id" class="form-control" onchange="LoadCategories()">
                    <option value=0> Seleccione la Categoria ... </option>
                    @foreach ($data['categories'] as $category)
                        <option value="{{ $category->id }}"
                            @if (isset($data['product'])) @if ($data['product']->category_id == $category->id)
                                    selected @endif
                            @endif
            ...
            <div class="col-sm-8 col-md-2">
                <input class="form-control" name="label-group" id="label-group" type="text"
                    value="{{ old('label-group', isset($data['product']) ? $data['product']->ProductCategory->ProductGroup->description : '') }}"
        ...
        <div class="row">
            ...
            <div class="col-sm-7 col-md-2">
                <select name="tax_id" id="tax_id" class="form-control">
                    ...
                    @foreach ($data['taxes'] as $tax)
                        <option value="{{ $tax->id }}"
                            @if (isset($data['product'])) @if ($data['product']->tax_id == $tax->id)
                                        selected @endif
                            @endif
            ...
            <div class="col-sm-4 col-md-2 ">
                <input class="form-control" name="cost_price" id="cost_price" type="number" step="any"
                    value="{{ old('cost_price', isset($data['product']) ? $data['product']->cost_price : '') }}"
                ...
                @if ($data_common['purchase_coin_symbol'] != $data_common['base_coin_symbol'])
                    <input class="form-control" name="cost_price_other" id="cost_price_other" type="number"
                        step="any"
                        value="{{ old('cost_price', isset($data['product']) ? number_format($data['product']->cost_price * $data_common['purchase_rate'], 2) : '') }}"
                        required aria-required="false"
            ...
            <div class="col-sm-3">
                <input class="form-control {{ $errors->has('sale_price') ? ' is-invalid' : '' }}" name="sale_price"
                    ...
                    value="{{ old('sale_price', isset($data['product']) ? $data['product']->sale_price : '') }}"
                    required aria-required="false"
                ...
                @if ($data_common['sale_coin_symbol'] != $data_common['base_coin_symbol'])
                    <input class="form-control {{ $errors->has('sale_price') ? ' is-invalid' : '' }}"
                        ...
                        value="{{ old('sale_price', isset($data['product']) ? number_format($data['product']->sale_price * $data_common['sale_rate'], 2) : '') }}"
                        required aria-required="false"
    ...
    <div class="col-sm-2 border">
        ...
        <input type="file" name="imagefile" id="imagefile" accept="image/*" onchange="preview(event)"
            style="display:none"
            value="{{ old('image_file', isset($data['product']) ? $data['product']->image_file : '') }}">

        <div id="display-image">
            <img src="{{ isset($data['product']) ? asset($data['product']->image_file) : '' }}"
                alt="{{ isset($data['product']) ? 'Imagen no Disponible' : 'Seleccione una Imagen' }} "
