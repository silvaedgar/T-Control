<div class="row">
    ...
    <input type="hidden" value ="{{isset($productcategory) ? old('user_id',$productcategory->user_id) : auth()->user()->id }}" name = "user_id">
            ...
            @foreach ($groups as $group)
                <option
                    @if (isset($productcategory))
                        @if ($group->id == $productcategory->group_id)
                            selected
                        @endif
                    @endif
    ...
    <div class="col-sm-8">
      <div class="form-category{{ $errors->has('description') ? ' has-danger' : '' }}">
        ...
        placeholder="{{ __('Descripción') }}" value="{{ old('description',(isset($productcategory) ? $productcategory->description : '')) }}" required="true" aria-required="true"/>
