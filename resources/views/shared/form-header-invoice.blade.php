            ...
            <div class="col-sm-10 col-md-3 col-lg-4 ml-md-2 mt-md-3 ">
                @if ($controller == 'Sale')
                    <select name="client_id" id="client_id" class="form-control"
                        ...
                        value={{ old('client_id', isset($invoice) ? $invoice->client_id : '') }}>
                ...
                @else
                    <select name="supplier_id" id="supplier_id" class="form-control"
                        {{ isset($invoice) ? 'disabled' : '' }}
                        value={{ old('supplier_id', isset($invoice) ? $invoice->supplier_id : '') }}>
            ...
            <div class="col-sm-4 col-md-3 col-lg-2  ml-sm-2 mt-sm-2">
                @if ($controller == 'Sale')
                    <input type="date" name="sale_date" id="sale_date" class="form-control"
                        {{ isset($invoice) ? 'disabled' : '' }}
                        value="{{ old('sale_date', isset($invoice) ? $invoice->sale_date : date('Y-m-d')) }}">
                ...
                @else
                    <input type="date" name="purchase_date" id="purchase_date" class="form-control"
                        {{ isset($invoice) ? 'disabled' : '' }}
                        value="{{ old('purchase_date', isset($invoice) ? $invoice->purchase_date : date('Y-m-d')) }}">
        ...
        <div class="row">
            ...
            <div class="col-sm-2 col-md-1 col-xl-1 ml-sm-3">
                <select name="coin_id" id="coin_id" class="form-control"
                    ...
                    @foreach ($coins as $coin)
                        <option value="{{ $coin->id }}" @php
                            $selected = old('coin_id') == 0 && $calc_coin_id == $coin->id && !isset($invoice) ? true : false;
            ...
            <div class="col-sm-2 mt-sm-2">
                <input type="number" name="rate_exchange" id="rate_exchange" class="form-control" step="any"
                    {{ isset($invoice) ? 'disabled' : '' }} onfocus="VerifyGetFocusRateExchange()"
                    onkeyup="RecalculateInvoice(false); CalcSubtotal('Price')"
                    value="{{ isset($invoice) ? $invoice->rate_exchange : 1 }}">
            ...
            <div class="col-sm-4  col-md-3 col-xl-2 mt-sm-3 ml-md-4" {{ isset($invoice) ? 'disabled' : '' }}>
