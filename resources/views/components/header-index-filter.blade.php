<div class="row">
    @if ($config['cols'] == 2)
        ...
        <div class="col-sm-3 col-md-5 col-lg-{{ !$config['index'] ? 8 : 4 }}">
            <h4 class="card-title"> {{ $config['header']['title'] }} Edgar</h4>
            @foreach ($config['links_create'] as $key => $link)
                <a style="color: #99ffff; font-size: 12px" href="{{ $link['url'] }}">
                    {{ $key > 0 ? ' | ' : '' }} {{ $link['message'] }} </a>
            @endforeach
        </div>
        <div class="col-sm-8 col-md-6 col-lg-{{ !$config['index'] ? 4 : 8 }}">
            @if (!$config['index'])
            ...
            @else
                @foreach ($config['buttons'] as $key => $button)
                    <a class="float-end" href="{{ $button['url'] }}" target="{{ $button['target'] ? '_blank' : '' }}">
                        <button class="btn btn-info"> {{ $button['message'] }}1
